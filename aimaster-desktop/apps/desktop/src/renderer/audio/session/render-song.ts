...
export async function renderSong(input: RenderSongInput): Promise<RenderSongOutput> {
  ...
  if (!input.settings) {
    const result = await api.invoke(
      ...
      {
        style:              options.style,
        // Only when the user actually chose a number.  Sending the default
        // every time tells the engine a target was picked when none was, and
        // the classic path then normalises to it instead of leaving the song
        // where its own analysis put it.
