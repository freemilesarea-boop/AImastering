...
export type DawWindow = 'edit' | 'mix' | 'midi' | 'chain' | 'session' | 'spectral' | 'reference' | 'warp' | 'restore' | 'steps';
...
export const useDawStore = create<DawState>((set, get) => ({
  ...
  toggleWindow: () => set((s) => {
    const order: DawWindow[] = ['edit', 'mix', 'midi', 'chain', 'session', 'steps', 'warp', 'spectral', 'restore', 'reference'];
