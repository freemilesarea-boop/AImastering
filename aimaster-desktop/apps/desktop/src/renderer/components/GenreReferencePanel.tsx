...
export default function GenreReferencePanel(): React.ReactElement {
  ...
  if (!analysis) return <p className="text-[11px] text-zinc-400">먼저 파일을 분석하면 장르를 추정합니다.</p>;
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-zinc-300">신뢰도 {conf}%</span>
        </div>
        <button type="button" onClick={() => setScanTick((n) => n + 1)} className="text-[11px] text-zinc-300 hover:text-zinc-300"
          title="재생 중 누르면 스펙트럼으로 정밀 재추정">↻ 재추정</button>
      </div>

      <div className="flex gap-1 flex-wrap">
        {genre.candidates.map((c) => (
          <span key={c.genre} className={`text-[11px] px-1.5 py-0.5 rounded border ${c.genre === genre.top ? 'border-indigo-500/50 text-indigo-300' : 'border-zinc-700 text-zinc-300'}`}>{c.label}</span>
        ))}
      </div>

      <div className="rounded border border-zinc-800 p-2 space-y-1.5">
        <div className="flex items-center justify-between">
          <span className="text-[11px] text-zinc-300">추천 모드</span>
      ...
      <div className="space-y-1">
        <p className="text-[11px] text-zinc-400 uppercase tracking-wider">가까운 아시아 레퍼런스 (톤 fingerprint)</p>
        {refs.map((m) => (
          <div key={m.ref.id} className="flex items-center justify-between rounded border border-zinc-800 px-2 py-1">
            <div className="flex items-center gap-2">
              <span className="text-[11px] px-1 rounded bg-zinc-700 text-zinc-400">{REGION_LABEL[m.ref.region]}</span>
              <span className="text-[11px] text-zinc-200">{m.ref.name}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-mono text-zinc-300">{m.ref.lufs} LUFS · LRA {m.ref.lra}</span>
              <button type="button" onClick={() => updateOptions({ targetLufs: m.ref.lufs, targetTp: m.ref.truePeakDbtp })}
                className="text-[11px] text-indigo-400 hover:text-indigo-300">타겟 적용</button>
            </div>
          </div>
        ))}
      </div>
      <p className="text-[11px] text-zinc-400">레퍼런스는 저작권 안전한 톤 fingerprint(오디오·곡명 없음)입니다.</p>
    </div>
  );
}
