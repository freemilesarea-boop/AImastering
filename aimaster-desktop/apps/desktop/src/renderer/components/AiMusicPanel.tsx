...
import { detectAiMusic, buildAiMusicFeatures, refineWithSpectrum } from '../audio/ai-music.js';
import { getActiveSpectrumSnapshot } from '../audio/shared-audio-graph.js';
import { isUsableSpectrum } from '../audio/ai-music-spectral.js';
...
export default function AiMusicPanel(): React.ReactElement {
  ...
  const { report, refined } = useMemo(() => {
    if (!analysis) return { report: null, refined: false };
    const sb = masteringResult?.spectralBalance ?? null;
    const extra = sb ? { highToMidDb: sb.highToMidDb, lowToMidDb: sb.lowToMidDb } : {};
    let features = buildAiMusicFeatures(analysis, extra);
    // Refine with a live FFT snapshot when audio is flowing (best-effort).
    const frame = getActiveSpectrumSnapshot();
    const usable = isUsableSpectrum(frame);
    if (usable) features = refineWithSpectrum(features, frame);
    return { report: detectAiMusic(features), refined: usable };
    // scanTick forces a re-capture while previewing.
