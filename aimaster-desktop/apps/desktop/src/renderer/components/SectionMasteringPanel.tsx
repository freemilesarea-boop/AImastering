...
export default function SectionMasteringPanel(): React.ReactElement {
  ...
  if (!sections || sections.length === 0) {
    return <p className="text-[11px] text-zinc-600">구간 분석 결과가 없습니다. 먼저 파일을 분석하면 벌스/코러스별 게인을 조정할 수 있습니다.</p>;
  }
  ...
  return (
    <div className="space-y-2">
          ...
          return (
            <div key={`${sec.start}-${i}`}>
              <div className="flex items-baseline justify-between">
                <span className="text-[10px] text-zinc-400">{sec.label ?? kind} <span className="text-zinc-600">{fmtTime(sec.start)}–{fmtTime(sec.end)}</span></span>
                <span className="text-[10px] font-mono text-zinc-400">{gain > 0 ? '+' : ''}{gain.toFixed(1)} dB</span>
              </div>
              <input
      ...
      <p className="text-[10px] text-zinc-600">경계는 부드럽게 크로스페이드되며, 마스터링 체인이 구간을 자연스럽게 결합합니다.</p>
    </div>
  );
}
