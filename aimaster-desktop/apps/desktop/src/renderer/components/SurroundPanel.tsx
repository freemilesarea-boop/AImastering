      ...
      <div className={s.foldDownEnabled ? 'space-y-2' : 'space-y-2 opacity-50 pointer-events-none'}>
        ...
        <div className="flex gap-1 text-[11px]">
          {(['foldDown', 'multichannel'] as const).map((m) => (
            <button key={m} type="button" onClick={() => update({ mode: m })}
              className={`flex-1 rounded px-2 py-1 ${s.mode === m ? 'bg-indigo-600 text-white' : 'bg-zinc-700 text-zinc-400 hover:text-zinc-200'}`}>
        ...
        {!multichannel && (
          <div className="space-y-1.5">
            ...
            <p className="text-[11px] text-zinc-400">ITU-R BS.775 폴드다운 → 스테레오 체인으로 마스터.</p>
          </div>
        )}
        ...
        {multichannel && (
          <div className="space-y-1.5">
            ...
            <label className="flex items-center justify-between gap-2 text-[11px] text-zinc-400">
              ...
              <select value={s.dolbyCodec ?? 'none'} onChange={(e) => update({ dolbyCodec: e.target.value as 'none' })} aria-label="Dolby 코덱" className="bg-zinc-700 rounded px-1 py-0.5 text-[11px]">
            ...
            {s.perChannelChain && (
              <div className="space-y-2 rounded border border-zinc-800 p-2">
                <p className="text-[11px] text-zinc-300 uppercase tracking-wider">베드별 조정</p>
                {(['front', 'center', 'surround'] as const).map((bed) => (
                  <div key={bed} className="space-y-1">
                    <p className="text-[11px] text-zinc-400">{bed === 'front' ? '프론트' : bed === 'center' ? '센터' : '서라운드'}</p>
            ...
            <p className="text-[11px] text-zinc-400">레이아웃 보존 · (옵션) 베드별 풀 체인 + 베드별 톤/레벨 → 목표 LUFS 자동매칭(BS.1770) + 마스터 게인 트림 + 채널 링크드 트루피크 리미터(이미징 보존). 출력은 멀티채널 WAV.</p>
          </div>
        )}
      </div>
      <p className="text-[11px] text-zinc-400">채널 기반 서라운드(객체 기반 Atmos 오서링 아님). 스테레오 소스는 영향 없음.</p>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="flex justify-between items-baseline">
        ...
        <span className="text-[11px] font-mono text-zinc-400">{value > 0 ? '+' : ''}{value.toFixed(1)}</span>
      </div>
