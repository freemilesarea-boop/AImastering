...
export default function MasteringHistoryPanel(): React.ReactElement {
  ...
  if (entries.length === 0) {
    return <p className="text-[11px] text-zinc-400">아직 마스터링 이력이 없습니다. 마스터링을 실행하면 설정과 측정값이 여기에 저장됩니다.</p>;
  }

  return (
    <div className="space-y-2">
      <p className="text-[11px] text-zinc-400">{entries.length}개 · 설정·측정값만 저장 (오디오 파일 아님)</p>
      <ul className="space-y-1.5">
        {entries.map((e) => (
          <li key={e.id} className="rounded border border-zinc-800 bg-zinc-800/40 px-2 py-1.5">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className={`text-[11px] leading-none ${e.isFavorite ? 'text-amber-400' : 'text-zinc-400 hover:text-zinc-400'}`}
                  >
                    {e.isFavorite ? '★' : '☆'}
                  </button>
                  <span className="truncate text-[11px] text-zinc-200" title={e.label}>{e.label}</span>
                </div>
                <p className="truncate text-[11px] text-zinc-300" title={e.sourceFileName}>{e.sourceFileName} · {timeAgo(e.createdAt)}</p>
              </div>
              <div className="flex shrink-0 items-center gap-1.5">
                <button type="button" onClick={() => restore(e.id)} className="rounded bg-indigo-600/80 px-1.5 py-0.5 text-[11px] text-white hover:bg-indigo-600">설정 복원</button>
                <button
                  type="button"
                  onClick={() => { const v = window.prompt('이름 변경', e.label); if (v) rename(e.id, v); }}
                  className="text-[11px] text-zinc-300 hover:text-zinc-300"
                >이름</button>
                <button type="button" onClick={() => remove(e.id)} aria-label="삭제" className="text-[11px] text-zinc-400 hover:text-red-400">✕</button>
              </div>
            </div>
            <div className="mt-1 flex flex-wrap gap-x-2 text-[11px] font-mono text-zinc-300">
              <span>{formatMetrics(e.metrics)}</span>
              <span className="text-zinc-400">·</span>
              <span>{formatOptionsSummary(e.optionsSnapshot)}</span>
              {e.formatSummary && <><span className="text-zinc-400">·</span><span>{e.formatSummary}</span></>}
            </div>
          </li>
        ))}
      </ul>
