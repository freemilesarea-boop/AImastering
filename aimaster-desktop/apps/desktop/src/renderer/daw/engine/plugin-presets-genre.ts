...
export const GENRE_PRESETS: readonly PluginPreset[] = [
  ...
  ...genrePresets('ducker', {
    ...
    edm:     { note: '12:1 · 어택 5 ms. 이 펌핑은 효과가 아니라 곡의 구조입니다',
               params: { thresholdDb: -16, ratio: 12, attackMs: 9, releaseMs: 90, makeupDb: 3 } },
  ...
  ...genrePresets('gate', {
    jazz:    { note: '레인지 12 dB — 완전히 닫지 않습니다. 방 소리도 연주의 일부입니다',
               params: { thresholdDb: -52, rangeDb: 12, attackMs: 9, releaseMs: 400 } },
    ...
    kpop:    { note: '45 dB · 어택 2 ms. 트랙 사이가 완전히 조용해야 합니다',
               params: { thresholdDb: -36, rangeDb: 45, attackMs: 9, releaseMs: 120 } },
    pop:     { note: '34 dB. 지우되 티는 안 나게',
               params: { thresholdDb: -40, rangeDb: 34, attackMs: 9, releaseMs: 180 } },
    edm:     { note: '55 dB · 1 ms. 게이트가 아니라 스위치처럼 씁니다',
               params: { thresholdDb: -32, rangeDb: 55, attackMs: 9, releaseMs: 90 } },
    hiphop:  { note: '48 dB. 랩 트랙의 숨소리는 남기고 방 소리만 지우려면 임계를 올리세요',
               params: { thresholdDb: -34, rangeDb: 48, attackMs: 9, releaseMs: 110 } },
    ...
    jpop:    { note: '40 dB. 파트가 많을수록 안 쓰는 트랙이 조용해야 합계가 깨끗합니다',
               params: { thresholdDb: -38, rangeDb: 40, attackMs: 9, releaseMs: 150 } },
  }),
  ...
  ...genrePresets('mbwidth', {
    jazz:    { note: '120 Hz 아래만 모노. 위는 마이크가 잡은 그대로 둡니다',
               params: { lowXHz: 120, highXHz: 5000, lowWidth: 0, midWidth: 1, hiWidth: 1.05 } },
    lofi:    { note: '오히려 좁힙니다 — 좁은 것이 이 장르의 소리입니다',
               params: { lowXHz: 160, highXHz: 3000, lowWidth: 0, midWidth: 0.8, hiWidth: 0.85 } },
    ambient: { note: '가장 넓게. 200 Hz 아래만 붙잡아 두면 위는 얼마든지 벌어집니다',
               params: { lowXHz: 200, highXHz: 2500, lowWidth: 0, midWidth: 1.3, hiWidth: 1.7 } },
    classic: { note: '홀이 이미 폭을 만들었습니다 — 80 Hz 아래만 정리합니다',
               params: { lowXHz: 80, highXHz: 6000, lowWidth: 0.3, midWidth: 1, hiWidth: 1 } },
    kpop:    { note: '150 Hz 아래 모노, 4 kHz 위 1.5 배 — 가운데는 보컬 자리라 건드리지 않습니다',
               params: { lowXHz: 150, highXHz: 4000, lowWidth: 0, midWidth: 1, hiWidth: 1.5 } },
    pop:     { note: 'K-POP 에서 한 단계 덜어낸 것. 위를 1.3 배까지만',
               params: { lowXHz: 140, highXHz: 4500, lowWidth: 0, midWidth: 1, hiWidth: 1.3 } },
    edm:     { note: '180 Hz 아래를 완전히 모노로. 클럽 시스템은 저역을 합쳐서 냅니다',
               params: { lowXHz: 180, highXHz: 3500, lowWidth: 0, midWidth: 1.15, hiWidth: 1.45 } },
    hiphop:  { note: '808 을 정중앙에 박아두고 위만 엽니다',
               params: { lowXHz: 170, highXHz: 5000, lowWidth: 0, midWidth: 1, hiWidth: 1.25 } },
    rnb:     { note: '중역을 살짝 넓혀 코드가 감싸게. 저역은 그대로 가운데',
               params: { lowXHz: 130, highXHz: 3500, lowWidth: 0, midWidth: 1.2, hiWidth: 1.35 } },
    jpop:    { note: '편곡이 빽빽하니 위를 벌려 자리를 만듭니다',
               params: { lowXHz: 150, highXHz: 3800, lowWidth: 0, midWidth: 1.1, hiWidth: 1.55 } },
  }),

  ...genrePresets('upward', {
    jazz:    { note: '2 dB 만. 재즈에서 조용한 마디는 연주지 결함이 아닙니다',
               params: { thresholdDb: -28, ratio: 1.6, depthDb: 2, floorDb: -58,
                         attackMs: 80, releaseMs: 700 } },
    lofi:    { note: '플로어를 −44 dB 로 높게 잡습니다 — 테이프 히스를 같이 올리면 끝입니다',
               params: { thresholdDb: -24, ratio: 2, depthDb: 5, floorDb: -44,
                         attackMs: 60, releaseMs: 500 } },
    ambient: { note: '가장 깊고 가장 느리게. 꼬리가 사라지기 전에 받쳐 올립니다',
               params: { thresholdDb: -30, ratio: 2.6, depthDb: 9, floorDb: -62,
                         attackMs: 150, releaseMs: 1200 } },
    classic: { note: '1.5 dB. 홀의 잔향과 객석 소음이 바로 아래에 있습니다',
               params: { thresholdDb: -32, ratio: 1.4, depthDb: 1.5, floorDb: -54,
                         attackMs: 200, releaseMs: 1000 } },
    kpop:    { note: '6 dB 를 빠르게 — 무엇 하나 뒤로 물러나지 않는 것이 이 장르입니다',
               params: { thresholdDb: -20, ratio: 2.4, depthDb: 6, floorDb: -50,
                         attackMs: 25, releaseMs: 250 } },
    pop:     { note: 'K-POP 에서 한 단계 덜어낸 것. 4 dB 면 충분합니다',
               params: { thresholdDb: -22, ratio: 2, depthDb: 4, floorDb: -52,
                         attackMs: 40, releaseMs: 350 } },
    edm:     { note: '2.5 dB. 이미 평탄한 소재를 더 밀면 남는 건 룸톤뿐입니다',
               params: { thresholdDb: -18, ratio: 1.8, depthDb: 2.5, floorDb: -48,
                         attackMs: 30, releaseMs: 200 } },
    hiphop:  { note: '히트 사이에 깔린 것들을 올립니다. 킥이 지나간 자리가 비지 않게',
               params: { thresholdDb: -26, ratio: 2.2, depthDb: 5, floorDb: -56,
                         attackMs: 50, releaseMs: 300 } },
    rnb:     { note: '느리게 7 dB. 서스테인이 이 장르의 전부입니다',
               params: { thresholdDb: -26, ratio: 2.4, depthDb: 7, floorDb: -56,
                         attackMs: 100, releaseMs: 800 } },
    jpop:    { note: '편곡이 빽빽할수록 골이 깊습니다 — 팝보다 낮게 걸고 6.5 dB 를 올립니다',
               params: { thresholdDb: -27, ratio: 2.6, depthDb: 6.5, floorDb: -54,
                         attackMs: 35, releaseMs: 450 } },
  }),
  ...
  ...genrePresets('amp', {
    jazz:    { note: '한 단, 거의 클린. 재즈 기타는 앰프를 왜곡시키지 않습니다',
               params: { gain: 18, stages: 1, master: 25, sag: 15, cab: 0, mic: 65, treble: -2 } },
    lofi:    { note: '작은 앰프를 세게 — 두 단에 마이크를 축 위로 바짝',
               params: { gain: 72, stages: 2, master: 70, sag: 70, cab: 0, mic: 15, treble: -4, bass: -3 } },
    ambient: { note: '클린에 프레즌스만. 캐비닛을 끄고 뒤에 리버브를 두는 쪽',
               params: { gain: 12, stages: 1, master: 20, sag: 10, cab: 3, presence: 6, mic: 50 } },
    classic: { note: '기타 앰프는 클래식 악기가 아닙니다 — 최소한으로',
               params: { gain: 8, stages: 1, master: 15, sag: 0, cab: 0, mic: 70, level: -3 } },
    kpop:    { note: '단단한 크런치. 4×12 에 오프 액시스로 날카로움을 덜어냅니다',
               params: { gain: 55, stages: 2, master: 45, sag: 30, cab: 2, mic: 60, stack: 1, mid: -3 } },
    pop:     { note: '두 단 크런치, 2×12. 가장 평범하게 쓰이는 설정',
               params: { gain: 45, stages: 2, master: 40, sag: 35, cab: 1, mic: 45 } },
    edm:     { note: '세 단에 미드를 파냅니다 — 신스와 겹치지 않게',
               params: { gain: 85, stages: 3, master: 55, sag: 25, cab: 2, mic: 35, mid: -8, stack: 1 } },
    hiphop:  { note: '한 단, 새그를 끝까지. 샘플처럼 눌린 클린 톤',
               params: { gain: 25, stages: 1, master: 60, sag: 90, cab: 1, mic: 70, bass: 3 } },
    rnb:     { note: '따뜻한 클린에 미드를 살짝 올려 — 코드 커팅용',
               params: { gain: 22, stages: 1, master: 30, sag: 40, cab: 1, mic: 55, mid: 3, treble: -2 } },
    jpop:    { note: '밝은 크런치, 브리티시 스택. 트레블을 올려 앞으로',
  ...
  ...genrePresets('rotary', {
    ...
    lofi:    { note: '아주 느리게, 드라이브를 올려서. 테이프에 눌린 오르간',
               params: { rateHz: 0.45, throb: 60, doppler: 120, drive: 55, xoverHz: 650, mix: 100 } },
    ambient: { note: '거의 멈춘 회전에 마이크를 넓게. 음이 제자리에서 흔들립니다',
               params: { rateHz: 0.25, throb: 30, doppler: 60, micAngle: 150, drive: 0, mix: 85 } },
    classic: { note: '파이프 오르간에는 회전이 없습니다 — 아주 옅게만',
               params: { rateHz: 0.6, throb: 18, doppler: 35, drive: 0, micAngle: 60, mix: 45 } },
    kpop:    { note: '빠른 트레몰로 속도. 신스 코드를 살짝 흔드는 용도',
               params: { rateHz: 5.5, throb: 50, doppler: 90, accelSec: 0.8, drive: 15, mix: 55 } },
    pop:     { note: '중간 속도, 기본에 가깝게',
               params: { rateHz: 2.4, throb: 55, doppler: 100, drive: 20, mix: 80 } },
    edm:     { note: '가장 빠르게 · 스핀업을 거의 없애서 스위치처럼 바로 붙습니다',
               params: { rateHz: 7.2, throb: 75, doppler: 140, accelSec: 0.2, micAngle: 180, mix: 70 } },
    hiphop:  { note: '느린 회전에 드라이브를 세게. 샘플처럼 들리게 만드는 쪽',
               params: { rateHz: 0.55, throb: 65, doppler: 110, drive: 70, xoverHz: 500, mix: 100 } },
    rnb:     { note: '가스펠 오르간 — 코랄에서 트레몰로로 넘어가는 그 속도',
               params: { rateHz: 6.4, throb: 70, doppler: 130, accelSec: 2.2, drive: 30, mix: 100 } },
    jpop:    { note: '빠르되 얕게. 배경에서 반짝이기만 합니다',
               params: { rateHz: 4.8, throb: 40, doppler: 75, micAngle: 110, drive: 10, mix: 50 } },
  }),
